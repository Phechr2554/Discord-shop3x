...
```bash
git clone https://github.com/Phechr2554/Discord-shop3x.git && cd Discord-shop3x && chmod +x install_vps.sh && sudo ./install_vps.sh && cd ~
```
...
ถ้าเลือกเก็บ `.env` ไว้ภายหลัง ให้เข้า `menubot` แล้วเลือกเมนู `6) ตั้งค่า .env`
...
6. ตั้งค่า `.env` — ใช้รูปแบบ `Token = "ค่า"` และแก้ค่าอื่น ๆ ได้ในหน้าจอเดียว

`menubot` ใช้ไฟล์ตั้งค่าที่ `/etc/menubot.conf` จึงเรียกใช้ได้จากทุกที่โดยไม่ต้องอยู่ในโฟลเดอร์โปรเจกต์
